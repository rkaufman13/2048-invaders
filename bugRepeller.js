function preload() {
  ...
  function sortedEnemiesY(){
    const orderedByXCoord = gameState.enemies.getChildren().sort((a, b) => a.y - b.y);
    return orderedByXCoord;
  }
...
return a.y - b.y;
  ...
  const gameState = {
   ...
   toprowcount:0,
   toprow: ["full","full","full","full","full","full","full","full"],
arraypos:0,
bottomrow: 8,
sumValueOfEnemies: 0,
scale: .5
  ...
  function create() {
      ...
      this.input.on('pointerup', () => {
          if (gameState.active === false) {
              ...
              gameState.toprowcount = 0;
              gameState.toprow = ["full","full","full","full","full","full","full","full"];
              gameState.arraypos = 0;
            gameState.bottomrow =8;
            gameState.sumValueOfEnemies = 0;
          }
      })
  
      // Creating static platforms
      const platforms = this.physics.add.staticGroup();
      platforms.create(225, 400, 'platform').setScale(1, .3).refreshBody();
  ...
  for (let yVal = 1; yVal <=2; yVal++){
      gameState.enemies.create(50 * xVal, 50 * yVal, '4').setScale(gameState.scale).setGravityY(-200);
    }
  }
  for (let xVal = 1; xVal <9; xVal++){
    for (let yVal = 3; yVal <=4; yVal++){
        gameState.enemies.create(50 * xVal, 50 * yVal, '2').setScale(gameState.scale).setGravityY(-200);
      }
    }
  ...
  return num;
   ...
   gameState.pelletsLoop = this.time.addEvent({
    ...
    loop: true,
      ...
      gameState.player = this.physics.add.sprite(225, 350, 'codey').setScale(.6);
    ...
    //commenting out the enemy pellet collider so that we can test the game without dying
   this.physics.add.collider(pellets,gameState.player,() => {
      gameState.active = false;
      gameState.pelletsLoop.destroy();
      this.physics.pause();
      this.add.text(100,250,'Game Over. Click to restart',{fontSize:'15px', fill: '#000'});
    });
...
if (bug.texture.key === "2048"){
  ...
  this.add.text(200,300,'You Win',{fontSize:'15px', fill: '#000'});
      ...
      //otherwise, they hit 2 different bugs, and we need to check to see if their values are equal
            ...
            let lowestBug = Math.ceil(sortedEnemiesY()[sortedEnemiesY().length-1].y/10)*10;
            console.log("first bug hit has a y value of " + oldBugY)
            console.log("second bug hit has a y value of " + yVal)
            console.log("the lowest enemy in the row has a y value of " + lowestBug)
            bug.destroy();
            bug2.destroy();
            gameState.whichBug = 0;
            gameState.enemies.create(xVal, yVal, doublebug).setScale(gameState.scale).setGravityY(-200);
          gameState.randomspawncounter++;
            ...
            if (oldBugY === lowestBug && yVal === lowestBug ){
              ...
              gameState.bottomrow --;
              console.log("I believe there are " + gameState.bottomrow + " enemies remaining at the bottom");
            }
            ...
            if (gameState.bottomrow === 0){
              console.log("row empty, moving everybody down")
              for (i = 0; i < sortedEnemies().length; i++){
                if (sortedEnemies()[i].y === Math.ceil(sortedEnemiesY()[sortedEnemiesY().length-1].y/10)*10) {
                gameState.bottomrow++;
                }
              
              }
              gameState.enemies.getChildren().forEach(bug => bug.y = bug.y+50);
              console.log("new bottom row has " + gameState.bottomrow + " enemies")
            }
            ...
            if (gameState.randomspawncounter<=3){
              ...
              //spawn another number
                ...
                if (gameState.toprow.indexOf(0) !=-1){
                  console.log("found at least one blank space in the top row");
                  //there are blank spaces in the array, so let's add a new tile
                
                   do {gameState.arraypos = RollAnNSidedDie(8); }
                  while (gameState.toprow[gameState.arraypos] === "full");  
                  yVal = sortedEnemiesY()[0].y;
                xVal = (gameState.arraypos * 50) + sortedEnemies()[0].x;
                gameState.enemies.create(xVal, yVal, getRandBug(bug)).setScale(gameState.scale).setGravityY(-200);
          ...
          gameState.toprow = [0,0,0,0,0,0,0,0];
